= OAuth support coming soon! Check out {Sirlantis' branch}[http://github.com/sirlantis/vimeo] in the mean time.
...
First, install {Gemcutter}[http://gemcutter.org], then you can:
...
All methods require authentication. Check out Vimeo's guides for {Web Based authentication}[http://www.vimeo.com/api/docs/web-auth] and {Desktop Based authentication}[http://www.vimeo.com/api/docs/desktop-auth].
...
You now have acquired the required OAuth credentials. Now you need to re-establish the OAuth connection and start
using the API.
...
=== Vimeo::Advanced::Album
  ...
  album.get_videos("user_id", { :page => "1", :per_page => "25", :full_response => "0", :password => nil })
  album.get_videos("album_id", "video_id")
